...
- テーマは ADR-007。色とダークの見た目は未決（Figma と独自スキーマが先）。実装は始めない
...
`App.vue` から `RecordInputView` を出す。コピー処理はストアの `copyFormattedText` を呼ぶ。テーマ用の composable は、色が決まってから足す。`useRecordStore` には置かない。
...
- ダーク（ADR-007）。色・ライトの作り直し・切替 UI は Figma と独自スキーマが固まってから。今は実装しない
