...
| 単位              | 責務                                                                                                                                 |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------ |
| `RecordInputView` | 画面全体。スコアと行動。コピー文面のライブプレビュー、注意文、常時有効のコピーと空にするボタン                                       |
...
正は Pinia の draft とコピー文面である。画面の文字はアプリが出す。

- draft とコピーは 24 時間の `HH:MM`（例: `08:30` `19:00`）。秒は持たない。タイムゾーン名は付けない
- 未設定は `null`。コピーとプレビューは `—`。モーダルを開いただけでは draft に書かない
- 未設定で開いたときは、開いた瞬間のブラウザ時計を初期値にする。既に値がある枠を開き直したときはその値
- 決定で `HH:MM` を draft に書く。クリアは `null`。キャンセルと背景クリックは変えない
...
- 時計の絵は Lucide の `clock`（ADR-014）。ブラウザ標準のインジケータは出さない。欄全体がタップ範囲
