...
| バックエンド           | Rust                    | 実装言語。フレームワーク、ホスト、認証、保存などの技術要素は US-08 着手時に検討する。いまの公開面は静的な Vercel（ADR-013） |
...
- 上流は Cursor と相談してよいが自分主導。下流の第一稿は Cursor でよい。生成物は [ADR-012](./adr/012-ai-usage-and-generated-code.md)
- バックエンドの実装言語は Rust とする。フレームワーク、ホスト、認証、保存などの技術要素は US-08 着手時に検討する。いまの公開面は静的な Vercel（ADR-013）
