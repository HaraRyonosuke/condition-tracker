...
未回答は 0 点と区別する（`null`）。7問すべて答えてからコピーできる。
...
| 単位 | 責務 |
|------|------|
| `RecordInputView` | 画面全体。合計の表示、コピーボタン、未回答時はコピー不可 |
| `ScoreForm` | 7項目を定義表の順に並べる |
| `ScoreItemInput` | 1項目の質問文と 0〜3 の4ボタン |
| `useRecordStore` | 入力中の draft（各項目は `number \| null`）。全問後に合計。`copyFormattedText` で日時を付けてクリップボードへ書く |
...
- 合計: `X/21`。区分（低 / 軽度 / 中等度 / 要注意）は出さない
- `[症状]` の7行。形式は `番号. 短いlabel: X/3`
...
時刻の分はゼロ埋めする。入力の4択ラベル（なし／少し／かなり／とても）は画面用であり、コピーには `X/3` だけを出す。
