...
- 長所: 既存の GitHub Actions で Node 26.9.0 を使ってビルドできる。HTTPS が付く。追加のホスト用ビルド環境が要らない
...
- 短所: 本番ビルドで使える Node は 20 / 22 / 24 系であり、26.9.0 ではない。`engines.node` が `^26.9.0` だとホスト側ビルドと衝突しうる。版を 24 系に下げると ADR-004 と CI が割れる
...
`main` への反映後、GitHub Actions が Node 26.9.0 でビルドし、静的ファイルを Pages へ出す。
...
効く制約は、ビルドの Node を 26.9.0 のままにすることである。
...
- ビルドは GitHub Actions 上の Node 26.9.0。ホスト側に実行時 Node は求めない
