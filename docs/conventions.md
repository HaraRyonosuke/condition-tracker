...
```
src/
├── views/                 # 画面（RecordInputView のみ）
├── components/            # 機能部品（画面・ドメイン固有）
│   └── common/            # 複数画面で使う汎用部品。最初の1個を足すときに作る
├── composables/           # 記録 draft 以外の画面寄りの状態。テーマ切替 F を置く。空フォルダは置かない
├── stores/                # Pinia
...
- UI 文言は日本語。絵文字は使わない。アイコンは Lucide（ADR-014）。食事時刻の時計は `clock`。ほかのグリフは使う画面で決める。
- 日付ピッカーは置かない。記録日時はコピー時にブラウザ時計から付ける。
- 食事時刻はモーダル。画面はアプリの 12 時間表示（午前／午後と時:分を中央揃え）。draft とコピーは 24 時間の `HH:MM`。欄全体がタップ範囲で OS のピッカーを開く。未設定で開いたときは開いた時刻。12／24 切替は持たない。正は [design.md](./design.md) 第7節。
- テーマは ADR-007。色は D3 オリーブ茶。切替は F。`useRecordStore` には置かない。`v-html` は使わない。

### データとストア

- チェック項目の `id`（`q1` … `q7`）は記録のキーとして安定させる。表示名や質問文を変えても `id` は変えない。
- 定義は `src/constants/checkItems.ts` の1か所。各記録は `{ key, score }` 相当だけを持つ。
- Pinia は Setup Store（`defineStore('id', () => { ... })`）。永続化しない（persist プラグインも localStorage も使わない）。
- ストアは入力中の draft（スコアと行動）とコピー用処理。履歴配列は持たない。テーマは置かない。
- 未回答は `null`。未回答を 0 として合計しない。食事の未設定も `null`。
...
- クリップボードや日時の副作用はストア／util に寄せ、表示コンポーネントは入力と表示に留める。テーマの `html` 操作は composable に寄せる。
