...
**ステータス: 一部確定。** 見た目と未使用変数は ESLint + Prettier（ADR-008）に任せる。Vue の書き方は [Vue.js 公式 Style Guide](https://vuejs.org/style-guide/) の Priority A / B を土台にし、対話で閉じた項目だけ下に記録する。未決は既定を仮置きしない。この文書の更新だけでは `src/` を直さない。食い違う箇所の適用と Cursor ルール（`.cursor/rules`）は別ターン。
...
次は対話で閉じていない。仮の既定をロックしない。

- **複数属性の意味上の並び**（定義 → リスト描画 → 条件、など）。折返しは Prettier に任せる。人が揃える意味順は、実際に揺れが出てから見直す。

## 6. あとでやること（今はやらない）

- 必要なら短い Cursor ルールを足す。
- 未決が閉じたら、この文書のステータスと該当節を更新する。
