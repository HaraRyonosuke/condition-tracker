...
- コードの書き方は Cursor ルールに置かない。Git の手順、UI 確認、ADR の提案、conversation の返信は [`.cursor/rules/`](../.cursor/rules/) と [`.cursor/hooks/`](../.cursor/hooks/) に置く（[PR merge](../.cursor/rules/pr-merge.mdc)、[ブランチ名](../.cursor/rules/branch-names.mdc)、[push 前リファクタ](../.cursor/rules/pre-push-refactor.mdc)、[UI 確認](../.cursor/rules/ui-canvas.mdc)、[ADR の提案](../.cursor/rules/adr-proposal.mdc)、[conversation の返信](../.cursor/rules/pr-conversation-reply.mdc)）。切り方の正は第7節。
...
範囲は、今回の変更が触ったファイルだけである。範囲の外は直さない。
...
lint / format が通っていることは、このリファクタが終わったことを意味しない。直すところがなければ、空のコミットは作らない。

### 今はやらない

- スプリントを束ねる中間ブランチ（`sprint/` や `develop`）は置かない。関わる人が増えたら、この節に戻して決める。

## 7. ブランチの切り方

既存ブランチは改名しない。中間ブランチは今は置かない（第6節の「今はやらない」）。コミットメッセージ規約は広げない。
