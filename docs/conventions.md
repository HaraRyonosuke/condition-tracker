...
├── composables/           # 記録 draft 以外の画面寄りの状態。色が決まってテーマを足すときに作る。空フォルダは置かない
├── stores/                # Pinia
...
- UI 文言は日本語。絵文字は使わない。
- 日付ピッカーは置かない。記録日時はコピー時にブラウザ時計から付ける。食事時刻はモーダルのネイティブ時刻入力（未設定で開いたときは開いた時刻）。
- テーマは ADR-007。色が決まるまでダークもライトの作り直しも実装しない。`useRecordStore` には置かない。`v-html` は使わない。
...
- クリップボードや日時の副作用はストア／util に寄せ、表示コンポーネントは入力と表示に留める。テーマの `html` 操作は、色が決まってから composable に寄せる。
