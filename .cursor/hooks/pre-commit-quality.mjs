 ...
 * 2. Check once. Failure denies this commit. The next git commit runs the hook again.
...
process.on('uncaughtException', (error) => {
  respond({
    permission: 'deny',
    user_message: 'コミットフックが失敗したため、コミットを止めました。',
...
function runCommitCheck() {
  ...
  // 2) Verify once. A failure denies this commit. The next git commit runs the hook again.
